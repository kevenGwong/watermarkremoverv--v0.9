...
class ZitsProcessor:
    ...
    def _load_model(self):
        ...
        try:
            ...
            # 加载ZITS模型
            self.model_manager = ModelManager(name="zits", device=str(self.device))
            ...
            logger.info(f"✅ ZITS model loaded successfully")
            logger.info(f"   Device: {self.device}")
            
        except Exception as e:
            logger.error(f"❌ Failed to load ZITS model: {e}")
            self.model = None
            self.model_loaded = False
    ...
    def predict(self, image: Image.Image, mask: Image.Image, config: Dict[str, Any] = None) -> np.ndarray:
        ...
        if not self.model_loaded:
            raise RuntimeError("ZITS model not loaded")
        ...
        try:
            ...
            logger.info(f"🎨 ZITS processing: {image_array.shape}")
            ...
            logger.info(f"✅ ZITS processing completed")
            return result
            
        except Exception as e:
            logger.error(f"ZITS prediction failed: {e}")
            raise
    ...
    def cleanup_resources(self):
        ...
        try:
            ...
            logger.info("✅ ZITS processor resources cleaned up")
        except Exception as e:
            logger.warning(f"⚠️ Error during ZITS processor cleanup: {e}") 