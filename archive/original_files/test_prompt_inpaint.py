...
def process_image_with_prompt(image, mask, model_manager, prompt="", negative_prompt="", dilate_kernel_size=4):
    """使用LaMa进行图像修复，支持prompt和mask膨胀"""
    # 对mask进行膨胀
...
def main():
    # 设置路径
    input_image_path = "/home/duolaameng/SAM_Remove/Watermark_sam/input/IMG_0001-3.jpg"
    ...
    test_cases = [
        {
            ...
            "prompt": "",
            "negative_prompt": "",
            "description": "无prompt（默认）"
        },
        {
            "name": "natural_prompt", 
            "prompt": "natural background, seamless, realistic",
            "negative_prompt": "watermark, text, logo, artificial",
            "description": "自然背景prompt"
        },
        {
            "name": "texture_prompt",
            "prompt": "texture, pattern, seamless",
            "negative_prompt": "watermark, text, logo",
            "description": "纹理prompt"
        },
        {
            "name": "clean_prompt",
            "prompt": "clean, smooth, uniform",
            "negative_prompt": "watermark, text, logo, noise",
            "description": "清洁prompt"
        }
    ]
    
    for test_case in test_cases:
        print(f"\n正在测试: {test_case['description']}")
        print(f"Prompt: '{test_case['prompt']}'")
        print(f"Negative prompt: '{test_case['negative_prompt']}'")
        ...
        # 保存结果
        output_path = Path(output_dir) / f"result_{test_case['name']}.jpg"
