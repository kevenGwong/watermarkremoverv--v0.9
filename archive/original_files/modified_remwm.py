...
def main(input_path: str, output_path: str, mask_path: str, overwrite: bool, transparent: bool, max_bbox_percent: float, force_format: str):
    ...
    def handle_one(image_path: Path, output_path: Path):
        ...
        # 使用自定义mask或自动生成mask
        if mask_path:
            mask_image = load_custom_mask(mask_path, image.size)
        ...
        if transparent:
            ...
            # 调用LaMa进行图像修复
            lama_result = process_image_with_lama(image_np, mask_array, model_manager)
            
            # 输出时转换BGR到RGB（iopaint API返回BGR格式）
